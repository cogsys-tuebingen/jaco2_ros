...
int main(int argc, char *argv[])
{
    ...
    std::vector<Jaco2Yaml2KDLTransform::KDLTransformation> tosave;
    for(int i = 0; i < 6; ++i)
    {
        ...
        try{
            ...
            Jaco2Yaml2KDLTransform::KDLTransformation t;
            t.name = accs[i];
            t.parent = links[i];
            KDL::Frame frame;
    ...
    if(tosave.size() == 6)
    {
        Jaco2Yaml2KDLTransform::save("/tmp/acc_transforms.yaml",tosave);
    }
